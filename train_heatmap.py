...
from net.twotasknet import Heatmap_net, Heatmap_net2
...
class Trainer(object):
    def __init__(self, args):
        ...
        ## model
        self.net = Heatmap_net2(cfg=self.cfg)
    ...
    def training(self):
        ...
        for epoch in range(args.epochs):
            for i, (data, label) in enumerate(self.train_data_loader):
                ...
                loss_all = self.heatmap_softiou(heatmap, label)
    ...
    def validation(self):
        ...
        for i, (data, label) in enumerate(self.val_data_loader):
            ...
            label = (label > self.cfg["label_vague_threshold"]).type(torch.float32)
            labels = self.heatmap_softiou.downsampleImg(label)
            for i in range(len(labels)):
                self.metric.update(labels[i].cpu(), pred[2 - i].cpu())
        ...
        self.writer.add_scalar("Best/mIoU", self.best, self.iter_num)
        self.writer.add_scalar("Best/Fmeasure", self.best, self.iter_num)
