...
from net.twotasknet import Heatmap_net, LocalSegment, LocalSegment2
...
def parse_args():
    ...
    # logger
    args.logger = setup_logger("LocalSegment test", args.save_folder, 0, filename="log.txt", mode="a")
    return args
...
class Trainer(object):
    def __init__(self, args):
        ...
        ## model
        self.net = LocalSegment2(cfg=self.cfg)
    ...
    def training(self):
        ...
        for epoch in range(args.epochs):
            for i, (data, label) in enumerate(self.train_data_loader):
                ...
                # local brightness normalize
                data_ = data.view(data.shape[0], -1)
                data_min,_ = torch.min(data_, dim=1)
                data_max,_ = torch.max(data_, dim=1)
                data = (data-data_min.view(-1,1,1,1))/(data_max-data_min+1e-5).view(-1,1,1,1)

                label = (label > self.cfg["label_vague_threshold"]).type(torch.float32)
                pred = self.net(data)
                loss = self.softiou(pred, label)
                ...
                self.writer.add_scalar('Train Loss/Loss All', np.mean(loss.item()), self.iter_num)
